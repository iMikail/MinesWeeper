...
class DifficultyButton: UIButton {
     ...
     func configureButton(difficulty: Difficulty) {
        if var configuration = configuration?.updated(for: self) {
            ...
            configuration.attributedSubtitle = AttributedString("\(difficulty.fieldDifficulty.fieldSize.row)x\(difficulty.fieldDifficulty.fieldSize.section), \(difficulty.fieldDifficulty.bombsCount)")
